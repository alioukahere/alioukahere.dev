...
Every time we need to add a new payment gateway, we have to modify this service. That's not great, we're violating the [Open/Closed Principle of SOLID](/blog/mastering-solid-principles-in-php). Today, we're going to fix it using the Adapter Pattern.
...
![Power Adapter Image](https://m.media-amazon.com/images/I/41TpcSAICUL._AC_UF1000,1000_QL80_.jpg)
...
2. **Adding new gateways means modifying the service**: Want to add CinetPay? You need to add a new private method, update the `match` statement, and add another API key to the constructor.
...
DTOs create a clean boundary between your application and external services. They make your contracts explicit and provide a single place to validate and transform data. Don't just rely on arrays (develop a little bit...)
