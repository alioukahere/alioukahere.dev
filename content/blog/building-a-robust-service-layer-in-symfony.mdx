...
Let's look at a real-world example. Imagine you're building a payment processing system that needs to support multiple payment methods: Wave, and Orange Money.
...
class PaymentController extends AbstractController
{
    ...
    public function processPayment(Request $request)
    {
        ...
        if (!in_array($paymentMethod, ['wave', 'orange'])) {
        ...
        if ($paymentMethod === 'wave') {
            $waveApiKey = $this->getParameter('wave.api_key');
            $waveBaseUrl = $this->getParameter('wave.base_url');

            $checkoutData = [
                'amount' => number_format($amount, 2, '.', ''),
                'currency' => 'XOF',
                'error_url' => $this->generateUrl('payment_error', [
                    'payment_method' => 'wave'
            ...
            try {
                $response = $this->httpClient->request('POST', $waveBaseUrl . '/v1/checkout/sessions', [
                    'headers' => [
                ...
                if (!isset($responseData['id']) || !isset($responseData['wave_launch_url'])) {
                    throw new \Exception('Invalid response from Wave API');
                }

                $externalPaymentId = $responseData['id'];
                $checkoutUrl = $responseData['wave_launch_url'];
            } catch (\Exception $e) {
                $this->logger->error('Wave checkout error', [
                    'user_id' => $user->getId(),
                    'error' => $e->getMessage(),
                ...
                return new JsonResponse([
                    'success' => false,
                    'error' => 'Wave payment error: ' . $e->getMessage()
                ], 500);
            }
        }

        if ($paymentMethod === 'orange') {
            // Another large block of code for Orange Money payment processing
        }
